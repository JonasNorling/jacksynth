...
TDelayFx::TDelayFx()
        : Delay(ms_to_samples(500)), Feedback(0.75), ReadPos(0)
{
    std::fill(Buffer[0], Buffer[0] + BufferSize, 0);
    std::fill(Buffer[1], Buffer[1] + BufferSize, 0);
}

void TDelayFx::Process(TSampleBufferCollection& in,
        TSampleBufferCollection& out)
{
    ...
    for (int i = 0; i < framesize; i++) {
        ...
        Buffer[0][writePos] = Feedback
                * (pingpong * (*in[0])[i] + (1 - pingpong) * Buffer[0][ReadPos]
                        + pingpong * Buffer[1][ReadPos]);
        (*out[0])[i] = (*in[0])[i] + Buffer[0][ReadPos];

        Buffer[1][writePos] = Feedback
                * ((1 - pingpong) * (*in[1])[i]
